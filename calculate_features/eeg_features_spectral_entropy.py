...
def calculate_hurst(raw, eid, columns):
    ...
    raw.info['bads'] = ['Oz', 'ECG']
    picks = mne.pick_types(raw.info, eeg=True, exclude='bads')
    entity = [raw.copy().filter(None, 4), raw.copy().filter(4, 7), raw.copy().filter(8, 10),
              raw.copy().filter(10, 12), raw.copy().filter(12, 13), raw.copy().filter(13, 40), raw.copy()]
...
def eeg_psd(control_raw, patient_raw):
    ...
    df.to_csv('data/after_c_features_nonliear_spectral_entropy.csv', index=True)
    ...
    for (eid, raw) in patient_raw.items():
        ...
        counter += 1
    df.reset_index(drop=True)
    df.to_csv('data/after_p_features_nonliear_spectral_entropy.csv', index=True)
    print(bigerror)
