...
def calculate_nonlinear_features(raw, eid, columns):
    ...
    for i in range(len(entity)):
        ...
        for j in range(pick_len):
            ...
            res[columns[j] + '_' + band_name[i] + '_' + 'hfd'] = hfd
...
def eeg_hfd(control_raw, patient_raw):
    """
    main function in this script to calculate hfd
    :param control_raw:
    :param patient_raw:
    :return:
    """
    ...
    df.to_csv(os.path.join(CONST.features_path, 'c_features_nonliear_hfd.csv'), index=True)
    ...
    for (eid, raw) in patient_raw.items():
        ...
        counter += 1
    df.reset_index(drop=True)
    df.to_csv(os.path.join(CONST.features_path, 'p_features_nonliear_hfd.csv'), index=True)
    print(bigerror)
